...
class LSTM(object):
	...
	def backward(self, dh):
		...
		# meta = [x,prev_h,next_h, prev_c, i_t, f_t, c_t, o_t, g_t]
		# Setting up Final memory derivatives
		# Get the Parameters from meta for the backward pass
		# o_t = self.meta[self.meta.length-1][-2]
		# c_t = self.meta[self.meta.length-1][-2]
		# dc0 = dh0 * o_t * (1.0 - np.tanh(c_t)**2)
